...
- Task: **1.1.4** — CI workflow written, awaiting commit. Stays [~] until web+api exist and the pipeline is green on main. Next: **1.2.1** Express+TS init
- Branch: `feature/repo-tooling`
- Notes: 1.1.1–1.1.3 merged to main (9a01b39) and pushed to origin (github.com/abirami2k1/knowHer). 1.2.6 (hardening) + 1.2.7 (Sentry, checkpoint) added to TASKLIST per founder OK. Local machine has Node 24 (plan says Node 20 LTS; .nvmrc set to 20). docker-compose.yml written but NOT run — founder starts it at the 2.1.1 checkpoint.
